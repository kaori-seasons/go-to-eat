...
from model_adapters_real import MiniCPM5RealAdapter
...
class RealTestRunner:
    ...
    def __init__(self):
        ...
        self.minicpm_adapter = MiniCPM5RealAdapter()
    ...
    async def run_single_test(
        ...
        try:
            ...
            if test_case.conversation_turns:
                return await self._run_conversation_test(test_case)

            # 单轮对话测试
            print(f"    发送请求到MiniCPM5-2B API...")
            response = await self.minicpm_adapter.process_message(
                message=test_case.input_message,
                available_tools=test_case.available_tools,
                context=test_case.context
            )

            print(f"    响应时间: {response.total_response_time_ms:.0f}ms")
            ...
            return TestExecutionResult(
                ...
                metrics=metrics,
                raw_api_response={"tokens_used": response.tokens_used}
            )
    ...
    async def _run_conversation_test(
        ...
        for turn in test_case.conversation_turns:
            ...
            print(f"    轮次 {turn['turn']}: {turn['user']}")
    ...
    async def run_all_tests(self) -> Dict[str, ModelMetrics]:
        ...
        """
        print("=" * 60)
        print("开始执行 MiniCPM5-2B 真实API测试")
        print("=" * 60)
        ...
        for i, test_case in enumerate(ALL_TEST_CASES, 1):
            ...
            minicpm_result = await self.run_single_test(test_case)
            ...
            print(f"    结果: 工具准确率={minicpm_result.metrics.get('tool_accuracy', 0):.2%}")
...
async def main():
    ...
    # 保存报告
    report_path = "/Users/kaori/Downloads/where-to-eat-main/tests/results/real_api_test_report.json"
    ...
    print(f"\n{'指标':<20} {'得分':<15}")
    print("-" * 40)
    print(f"{'工具准确率':<20} {minicpm['tool_accuracy']:.2%}")
    print(f"{'参数准确率':<20} {minicpm['param_accuracy']:.2%}")
    print(f"{'调用顺序':<20} {minicpm['sequence_accuracy']:.2%}")
    print(f"{'错误恢复':<20} {minicpm['error_recovery']:.2%}")
    print(f"{'上下文保持':<20} {minicpm['context_retention']:.2%}")
    print(f"{'约束遵守':<20} {minicpm['compliance_score']:.2%}")
    print(f"{'综合得分':<20} {minicpm['weighted_score']:.1f}")
